...
const EventImpact = () => {
    ...
    const change1M = eventData.map(event => event.Change_1M || 0);
    const change3M = eventData.map(event => event.Change_3M || 0);
    const change6M = eventData.map(event => event.Change_6M || 0);
    const cumulativeBefore = eventData.map(event => event["Cumulative Return Before"] || 0);
    const cumulativeAfter = eventData.map(event => event["Cumulative Return After"] || 0);
    ...
    return (
        <div className="event-impact-container">
            <h2 className="text-center mb-4">Event Impact Analysis on Brent Oil Prices</h2>

            <div className="chart-container mb-5">
                <h4 className="chart-title">Brent Oil Price Trends Around Events</h4>
                <Line data={priceTrendData} options={priceTrendOptions} />
            </div>

            <div className="chart-container mb-5">
                <h4 className="chart-title">Percentage Changes in Oil Prices</h4>
                <Bar data={percentageChangeData} options={percentageChangeOptions} />
            </div>

            <div className="chart-container mb-5">
                <h4 className="chart-title">Cumulative Returns Before and After Events</h4>
                <Bar data={cumulativeReturnData} options={cumulativeReturnOptions} />
            </div>

            <h4 className="text-center mb-4">Statistical Significance (T-Test)</h4>
            <Table striped>
                <thead>
                    <tr>
                        <th>Event</th>
